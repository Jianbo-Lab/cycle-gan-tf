...
def build_enc_dec(source,reuse=False) :
    ...
    with tf.variable_scope('decoder') as s:
        decoder_spec = []
        for l,(in_,out_,size_) in enumerate([(64,32,image_size//2),(32,16,image_size)]):
            decoder_spec += [
                TransposedConv2d('tconv_%d'%(l+1),in_,[batch_size,out_,size_,size_],3,3,2,2),
                InstanceNorm('tconv_in_%d'%(l+1)),
                Lrelu()
...
def build_critic(_t) :
    ...
    # ]
    for l,(in_,out_) in enumerate(
        [(channels,32),(32,64),(64,128),(128,256),(256,256)]):
        c_spec +=[
            Conv2d('conv2d_%d'%(l+1),in_,out_,4,4,2,2),
            #InstanceNorm('conv2d_in_%d'%(l+1)),
            Lrelu(),
    ...
    c_spec += [
        Linear('linear_1',256,256),
        Lrelu(),
        Linear('linear_2',256,256),
        Lrelu(),
        Linear('linear_3',256,1),
    ]
    for block in c_spec :
        _t = block(_t)
    return _t
